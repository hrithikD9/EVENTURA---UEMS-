...
function updateRecentRegistrants(registrants) {
  ...
  registrants.forEach(registrant => {
    ...
    // Create user initial avatar - safely handle potentially missing name
    let initials = 'U';
    if (registrant.name && typeof registrant.name === 'string') {
      initials = registrant.name.split(' ').map(n => n[0] || '').join('').toUpperCase() || 'U';
    }
...
// Add CSS for avatar circles and other dashboard styles
